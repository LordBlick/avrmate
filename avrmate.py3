...
pygtk.require('2.0')
try:
	from gi.repository import Gtk as gtk, GdkPixbuf, Pango
except:
	import gtk
	import pango
import gobject
...
class FuseAVR:
	...
	def uiInit(self):
		self.cfBPixbuf = GdkPixbuf.Pixbuf.new_from_file("pic/Configbyte.png")
		self.fsBPixbuf =  GdkPixbuf.Pixbuf.new_from_file("pic/Fusebyte.png")
		self.fsbPixbuf =  GdkPixbuf.Pixbuf.new_from_file("pic/Fusebit.png")
		self.lBPixbuf =  GdkPixbuf.Pixbuf.new_from_file("pic/Lockbyte.png")
		self.lbPixbuf =  GdkPixbuf.Pixbuf.new_from_file("pic/Lockbit.png")
		self.lghtPixbuf =  GdkPixbuf.Pixbuf.new_from_file("pic/Lightenings.png")
		
		gtk.Window.set_default_icon_list([self.fsBPixbuf, self.lghtPixbuf])
		
		self.title="AVRmate - Fusebits Calculator V.0.51"
		
		self.dialogWindow = gtk.Window(gtk.Window.TOPLEVEL)
...
## ## ## ## ## ## ##
		...
		self.buttonPng.connect("clicked", self.winShoot)
...
## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ##
	...
	def treeTest(self, widget):
		#self.avrdudeListuC()
		self.tvBits.set_cursor(-1, None, False)
		return
	...
	def winShoot(self, widget):
		width, height = self.dialogWindow.get_size()
		pixbuf = gtk.gdk.Pixbuf(gtk.gdk.COLORSPACE_RGB, True, 8, width, height)
		# Retrieve the pixel data from the gdk.window attribute (self.dialogWindow.window)
		# of the gtk.window object
		screenshot = pixbuf.get_from_drawable(self.dialogWindow.window, self.dialogWindow.get_colormap(), 
